 ...
 *
 * Stores credentials in an encrypted file at ~/.creator-flow/credentials.enc
...
// File location
const CREDENTIALS_DIR = join(homedir(), '.creator-flow');
...
export class SecureStorageBackend implements CredentialBackend {
  ...
  private getEncryptionKey(salt: Buffer): Buffer {
    ...
    const stableMachineId = createHash('sha256')
      ...
      .update('creator-flow-v2') // Bumped version for new key derivation
      .digest();
