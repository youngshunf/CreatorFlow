...
import { Tooltip, TooltipTrigger, TooltipContent, TooltipProvider, DocumentFormattedMarkdownOverlay } from "@sprouty-ai/ui"
import {
  DropdownMenu,
  DropdownMenuTrigger,
  DropdownMenuSub,
...
function AppShellContent({
  ...
  const { canGoBack, canGoForward, goBack, goForward, navigateToSource, navigateToSession, navigate: contextNavigate } = useNavigation()
  const t = useT()
  ...
  const handleDeleteSession = useCallback(async (sessionId: string, skipConfirmation?: boolean): Promise<boolean> => {
    ...
    if (result && wasSelected) {
      if (!sessionFilter || sessionFilter.kind === 'allSessions') {
        ...
        contextNavigate(routes.view.state(sessionFilter.stateId))
      } else if (sessionFilter.kind === 'label') {
        contextNavigate(routes.view.label(sessionFilter.labelId))
    ...
    return result
  }, [session.selected, onDeleteSession, sessionFilter, contextNavigate])
  ...
  const unifiedSidebarItems = React.useMemo((): SidebarItem[] => {
    ...
    // 3. Files, Marketplace, Sources, Skills, Subscription, Settings
  ...
  const listTitle = React.useMemo(() => {
    ...
    if (!sessionFilter) return t('所有对话')
    ...
    switch (sessionFilter.kind) {
      ...
      case 'label':
        return sessionFilter.labelId === '__all__' ? t('标签') : getLabelDisplayName(labelConfigs, sessionFilter.labelId)
      case 'view':
        return sessionFilter.viewId === '__all__' ? t('视图') : viewConfigs.find(v => v.id === sessionFilter.viewId)?.name || t('视图')
      default:
        return t('所有对话')
    }
    ...
    <AppShellProvider value={appShellContextValue}>
      ...
      <div className="h-full flex items-stretch relative">
              ...
              <div className="flex-1 flex flex-col min-h-0">
                ...
                <LeftSidebar
                  ...
                  links={[
                    ...
                    // --- Sessions Section (with Flagged, States, Labels as sub-items) ---
                    {
                      id: "nav:allSessions",
                      title: t('所有对话'),
                      label: String(workspaceSessionMetas.length),
                      icon: Inbox,
                      variant: sessionFilter?.kind === 'allSessions' && activeRoute !== 'home' ? "default" : "ghost",
        ...
        >
          ...
          <div
            style={isSessionsNavigation(navState) && isFilePreviewVisible ? { flex: 1 - filePreviewRatio } : undefined}
            className={cn(
            "overflow-hidden min-w-0 bg-foreground-2 shadow-middle",
            !(isSessionsNavigation(navState) && isFilePreviewVisible) && "flex-1",
            effectiveFocusMode ? "rounded-l-[14px]" : "rounded-l-[10px]",
            isRightSidebarVisible ? "rounded-r-[10px]" : "rounded-r-[14px]"
          )}>
            <MainContentPanel isFocusedMode={effectiveFocusMode} onUseMarketplaceApp={handleUseMarketplaceApp} />
          </div>
          )}

          {/* Right Sidebar - Inline Mode (≥ 920px) - hidden in files navigation */}
          {!effectiveFocusMode && !shouldUseOverlay && !isFilesNavigation(navState) && (
            <>
              {/* Resize Handle */}
              {isRightSidebarVisible && (
                <div
