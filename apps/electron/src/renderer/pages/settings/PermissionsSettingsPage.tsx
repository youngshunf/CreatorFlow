...
export default function PermissionsSettingsPage() {
  ...
  return (
    <div className="h-full flex flex-col">
      <PanelHeader title={t('权限')} actions={<HeaderMenu route={routes.view.settings('permissions')} helpFeature="permissions" />} />
          ...
          <div className="px-5 py-7 max-w-3xl mx-auto">
            <div className="space-y-8">
              {isLoading ? (
                ...
                <>
                  {/* About Section */}
                  <SettingsSection title={t('关于权限')}>
                    <SettingsCard className="px-4 py-3.5">
                      <div className="text-sm text-muted-foreground leading-relaxed space-y-1.5">
                        <p>
                          {t('权限控制您的智能体拥有多大的自主权。在探索模式下，智能体只能读取和研究，非常适合在提交更改之前理解问题。')}
                        </p>
                        <p>
                          {t('推荐的工作流程：先在探索模式下让智能体调查，审查提议的计划，然后自信地执行。')}
                        </p>
                        <p>
                          <button
                            type="button"
                  ...
                  <SettingsSection
                    title={t('默认权限')}
                    description={t('探索模式下允许的应用级模式。不在此列表中的命令将被阻止。')}
                    action={
                  ...
                  <SettingsSection
                    title={t('工作区自定义')}
                    description={t('扩展上述应用默认值的工作区级模式。')}
                    action={
                      (() => {
                    ...
                    <SettingsCard className="p-0">
                      {customPermissionsData.length > 0 ? (
                        <PermissionsDataTable
                          ...
                          maxHeight={350}
                          fullscreen
                          fullscreenTitle={t('工作区自定义')}
                        />
                      ) : (
                        <div className="p-8 text-center text-muted-foreground">
                          <p className="text-sm">{t('未配置自定义权限')}</p>
                          <p className="text-xs mt-1 text-foreground/40">
                            {t('在工作区中创建 permissions.json 文件以添加自定义规则。')}
                          </p>
                        </div>
                      )}
                    </SettingsCard>
