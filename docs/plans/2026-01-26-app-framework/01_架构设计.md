...
CreatorFlow 应用框架采用**三层架构 + 能力组合**的设计理念，旨在：
...
**组成**：
- **Skills**：独立的 AI 技能（如素材整理、脚本创作、选题研究）
- **Workflows**：独立的流程定义（如选题到发布流程）
...
**插件应用**：
- 可选，用于扩展主应用的能力
- 可以注入额外的 Skills、Workflows、UI 组件
- 可以扩展数据模型和集成能力
- 不能修改主应用的核心配置
...
```json
{
  "dependencies": {
    "platform": ">=1.0.0",
    "ui": [
      "ui.content-calendar@^1.0.0",
      "ui.post-editor@^2.0.0"
    ],
    "models": [
      "model.post@^1.0.0",
    ...
    "integrations": [
      "integration.xiaohongshu@^1.0.0",
      "integration.douyin@^1.0.0"
    ]
  },
  "capabilities": {
    "skills": [
    ...
    "workflows": [
      "workflow.topic-to-publish@^1.0.0"
    ]
  }
}
```
