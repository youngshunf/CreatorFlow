...
interface EventPayload {
  ...
  target?: string      // 目标插件 ID (可选，不指定则广播)
  type: string         // 事件类型
  data: any            // 事件数据
  timestamp: number
}
...
interface PluginPermissions {
  ...
  storage?: {
    quota?: number       // 存储配额 (bytes)
  }
  apis?: string[]        // 允许调用的系统 API
}
...
class PermissionManager {
  ...
  canUseStorage(pluginId: string, size: number): boolean {
    ...
    if (!perms?.quota) return size <= 10 * 1024 * 1024 // 默认 10MB
...
class SandboxedNetwork {
  ...
  async fetch(url: string, options?: RequestInit): Promise<Response> {
    ...
    headers.set('User-Agent', `Sprouty AI-Plugin/${this.pluginId}`)
...
class PluginCache {
  ...
  private maxSize = 50  // 最多缓存 50 个插件
...
本文档补充了平台层插件系统的关键技术细节：

1. **热重载**: 文件监听 + 模块缓存清除 + 状态恢复
2. **插件通信**: 事件总线 + RPC 调用
3. **沙箱隔离**: 权限系统 + API 包装
4. **性能优化**: 懒加载 + LRU 缓存
5. **安全性**: 代码签名 + 运行时监控

这些机制确保插件系统的**可靠性、安全性和开发效率**。
