...
```bash
# 验证技能包
creatorflow skill validate ./my-skill

# 发布新版本 (自动递增版本号)
creatorflow skill publish ./my-skill --bump patch
creatorflow skill publish ./my-skill --bump minor
creatorflow skill publish ./my-skill --version 2.0.0

# 批量发布 (CI/CD)
creatorflow skill publish-all ./skills-directory
```
      ...
      - name: Publish changed skills
        ...
        run: |
          for skill in ${{ steps.changes.outputs.skills }}; do
            creatorflow skill publish ./skills/$skill --bump patch
          done
```
